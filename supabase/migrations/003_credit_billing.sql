-- Credit wallets (one row per user). Ledger + optimizations for safe consume/refund.
...
-- Migrate legacy generation_credits into new wallet (best-effort).
insert into
  public.credit_wallets (user_id, credits_remaining, credits_purchased_total, credits_consumed_total)
select
  id,
  greatest(coalesce(generation_credits, 0), 0),
  greatest(coalesce(generation_credits, 0), 0),
  0
from
  public.profiles
...
-- Reserve one credit and create optimization row (single transaction).
...
begin
  ...
  insert into
    optimizations (user_id, resume_hash, jd_hash, status)
  values
    (
      p_user_id,
...
-- Finalize reserved credit after optimization succeeds or fails.
...
begin
  ...
  if p_success then
    ...
    where
      id = p_optimization_id;

    update credit_wallets
    set
      credits_reserved = credits_reserved - 1,
      credits_consumed_total = credits_consumed_total + 1,
      updated_at = now()
    where
      user_id = p_user_id
    ...
    insert into
      credit_transactions (user_id, type, credits, optimization_id)
    values
      (p_user_id, 'consume', -1, p_optimization_id);
  else
    update optimizations
    set
      status = 'failed',
    ...
    insert into
      credit_transactions (user_id, type, credits, optimization_id)
    values
      (p_user_id, 'refund', 1, p_optimization_id);
  end if;
...
-- Grant credits after verified payment (idempotent if payment already paid handled in app).
...
begin
  ...
  insert into
    credit_transactions (user_id, type, credits, payment_id)
  values
