      ...
      <div className="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8 lg:py-14">
        <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-10 xl:gap-12">
          <div className="text-center lg:text-left">
            ...
            <div className="mt-8 border-t border-slate-300/40 pt-8 sm:mt-9 sm:pt-9">
              <p className="text-sm font-semibold tracking-[-0.01em] text-indigo-800/90">
                {HOME_COMPARE_SECTION_EYEBROW}
              </p>
              <h2
                id="home-compare-heading"
                className="mt-1.5 text-pretty text-xl font-bold tracking-[-0.025em] text-slate-900 sm:text-2xl sm:leading-snug"
              >
                {HOME_COMPARE_SECTION_HEADING}
              </h2>
              ...
              <ol className="mx-auto mt-5 max-w-md list-none space-y-3 p-0 text-left text-sm leading-relaxed text-slate-700 lg:mx-0">
                {HOME_COMPARE_METRICS.map((metric, index) => (
                  <li key={metric.label} className="flex gap-3">
                    <span
                      className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${METRIC_BADGE_CLASSES[index] ?? METRIC_BADGE_CLASSES[3]}`}
                    >
                      {index + 1}
                    </span>
                    <span>
                      <strong className="font-semibold text-slate-900">{metric.label}:</strong>{" "}
                      {metric.body}
                    </span>
                  </li>
                ))}
